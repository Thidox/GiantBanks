...
public class Messages {
	...
	private void loadMain() {
		...
		if(list == null) {
			GiantBanks.log.log(Level.WARNING, "[" + plugin.getName() + "] There are no main messages specified in the msgTemplates.yml file. This might cause errors!");
			return;
		}
	...
	private void loadAdmin() {
		...
		if(list == null) {
			GiantBanks.log.log(Level.WARNING, "[" + plugin.getName() + "] There are no admin messages specified in the msgTemplates.yml file. This might cause errors!");
			return;
		}
	...
	private void loadErrors() {
		...
		if(list == null) {
			GiantBanks.log.log(Level.WARNING, "[" + plugin.getName() + "] There are no error messages specified in the msgTemplates.yml file. This might cause errors!");
			return;
		}
	...
	public Messages(GiantBanks plugin) {
		this.plugin = plugin;
		
		File configFile = new File(plugin.getDir(), "msgTemplate.yml");
		if(!configFile.exists()) {
			GiantBanks.log.log(Level.INFO, "[" + plugin.getName() + "] Extracting new msgTemplate.yml file...");
		...
		if(v < this.yamlVersion) {
			GiantBanks.log.log(Level.INFO, "[" + plugin.getName() + "] Your msgTemplate.yml has ran out of date. Updating now!");
	...
	public String getMsg(msgType type, String template, HashMap<String, String> data) {
		...
		if(Template != null) {
			for(Map.Entry<String, String> entry : data.entrySet()) {
				if(entry.getKey() != null && entry.getValue() != null) {
					...
					GiantBanks.log.log(Level.SEVERE, "[" + plugin.getName() + "] Message template engine received invalid parameter! (null)");
				}
			}
		...
		GiantBanks.log.log(Level.SEVERE, "[" + plugin.getName() + "] Template for " + template + " does not exist!");
