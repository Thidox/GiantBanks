...
public class dbUpdater {
	...
	public static void updateItemsMySQL(iDriver DB, double curV, double v) {
		if(curV < 1.1) {
		...
		GiantBanks.getPlugin().getLogger().log(Level.INFO, "Updating items table to version " + String.valueOf(v));
	}
	
	public static void updateItemsSQLite(iDriver DB, double curV, double v) {
		if(curV < 1.1) {
			DB.buildQuery("ALTER TABLE #__items \n", false, false, false, true);
