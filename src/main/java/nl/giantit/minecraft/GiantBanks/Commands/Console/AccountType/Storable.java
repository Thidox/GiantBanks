			...
			if(sH.contains(p.getName() + ".selectedType")) {
				...
				if(item == null) {
					if(id > 0) {
						...
						if(iH.isValidItem(id, type)) {
							...
							Heraut.say(p, mH.getConsoleMsg(msgType.ERROR, "itemInvalid"));
							return;
						}
					}else{
						Heraut.say(p, mH.getConsoleMsg(msgType.ERROR, "itemIDInvalid"));
						return;
					}
				}
				...
				if(item != null) {
					...
					case 0:
						...
						Heraut.say(p, mH.getConsoleMsg(msgType.ADMIN, "storableSet", d));
						break;
					case -1:
						Heraut.say(p, mH.getConsoleMsg(msgType.ERROR, "ItemInvalid"));
						break;
					case -2:
						...
						d.put("item", item);
						d.put("type", aT.getName());
						Heraut.say(p, mH.getConsoleMsg(msgType.ERROR, "storableFail", d));
						break;
					}
					return;
				}else{
